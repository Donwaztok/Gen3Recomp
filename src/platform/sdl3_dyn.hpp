...
// Resolve SDL3 entry points via dlopen("libSDL3.so.0") so they are never bound
// to gba-recomp's SDL2 (same symbol names, incompatible return values).
...
extern int (*SetHint)(const char* name, const char* value);
